...
csv_path   = 'batch4gabor_localizations_with_coords.csv'
north_csv   = '../data/north_DAS_multicoord.csv'
south_csv   = '../data/south_DAS_multicoord.csv'
...
ax.imshow(rgb, extent=extent_km, aspect='equal')               # bathymetry
ax.plot(df_north.x_km, df_north.y_km, c='red', label='North cable')
ax.plot(df_south.x_km, df_south.y_km, c='orange', label='South cable')
cnt = ax.contour(bathy, levels=levels,
                 colors='k', linestyles='--',
...
# colorbar 
cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap='plasma'),
                   ax=ax, pad=0.015, aspect=30, fraction=0.03)

cbar.set_label('Time [minutes]')  # Change label to minutes

# --- Define your markers mapping ---
markers = {
    'hf-pair': '*',
    'lf-pair': 'P',
    'north-hf': '+',
...
# --- Create one scatter artist per group (initially empty) ---
artists = {}
...
for (sensor, call), grp in groups:
    ...
    m = markers.get(lbl)
    sc = ax.scatter([], [], 
                    color=mpl.colormaps['plasma'](0),
                    marker=m, s=100, label=lbl)
    artists[lbl] = sc
ax.legend(loc='upper right', fontsize='small')
...
for (sensor, call), grp in groups:
    ...
    m = markers.get(lbl)
    sc = ax.scatter([], [], 
                    c=[], cmap='plasma', norm=norm,
                    marker=m, s=90, label=lbl)
    artists[lbl] = sc

# --- Prepare time frames ---
times = np.sort(df_valid['minutes'].unique())

# --- Initialization function ---
def init():
    for sc in artists.values():
...
def update(frame_time):
    df_frame = df_valid[df_valid['minutes'] <= frame_time]
    for (sensor, call), _ in groups:
        lbl = f"{sensor}-{call}"
        sc = artists[lbl]
        df_grp = df_frame[(df_frame['sensor'] == sensor) & (df_frame['call_type'] == call)]
        coords = np.column_stack((df_grp['x_km'], df_grp['y_km']))
        sc.set_offsets(coords)
        sc.set_array(df_grp['minutes'])

    current_utc = t0 + pd.Timedelta(minutes=frame_time)
    ax.set_title(f'UTC: {current_utc.strftime("%Y-%m-%d %H:%M")}')
    return tuple(artists.values())

# --- Build and save animation ---
anim = FuncAnimation(
    fig, update, frames=times[::2],
    init_func=init, blit=True, interval=1000/35
)

writer = FFMpegWriter(fps=35, bitrate=2400)
anim.save('localization_animation.mp4', writer=writer, dpi=300, savefig_kwargs={'transparent': True})
plt.close(fig)
